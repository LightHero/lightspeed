...
fn should_save_file_to_db() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let saved = file_store.save_file(save_repository.to_owned(), file_name.clone(), file_name.clone(), content_type, &binary_content).await?;
...
fn should_save_file_to_fs() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let read_content = file_store.read_file_content(&loaded.data.repository, &loaded.data.file_path).await.unwrap().read().await.unwrap();
...
fn should_save_file_to_db_with_specific_repo() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let saved_1 =
            file_store.save_file(save_repository_1.to_owned(), file_name_1.clone(), file_name_1.clone(), content_type.clone(), &binary_content).await?;
        let saved_2 =
            file_store.save_file(save_repository_2.to_owned(), file_name_2.clone(),file_name_2.clone(), content_type, &binary_content).await?;
...
fn save_should_fails_if_fs_repo_does_not_exist() -> Result<(), LsError> {
    tokio_test(async {
        ...
        assert!(
            file_store
                .save_file( save_repository_1.to_owned(), file_name_1.clone(),file_name_1.clone(), content_type.clone(), &binary_content)
                .await
                .is_err()
        );
...
fn should_save_file_to_db_with_relative_folder_in_repository() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let saved = file_store.save_file( save_repository.to_owned(), file_path.clone(),file_name.clone(), content_type, &binary_content).await?;
...
fn should_delete_file_from_db() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let saved = file_store.save_file( save_repository.to_owned(), file_path.clone(),file_name, content_type, &binary_content).await?;
...
fn should_allow_files_with_same_path_but_different_repository() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let saved_db_1 = file_store
            .save_file(repository_db_1.to_owned(), same_file_name.clone(),same_file_name.clone(), content_type.clone(), &binary_content)
            .await?;

        let saved_db_2 = file_store
            .save_file(repository_db_2.to_owned(), same_file_name.clone(),same_file_name.clone(), content_type.clone(), &binary_content)
            .await?;

        let saved_fs_1 = file_store
            .save_file(repository_fs_1.to_owned(),same_file_name.clone(),same_file_name.clone(), content_type.clone(), &binary_content)
            .await?;

        let saved_fs_2 = file_store
            .save_file(repository_fs_2.to_owned(),same_file_name.clone(),same_file_name.clone(), content_type.clone(), &binary_content)
            .await?;
...
fn should_fail_if_file_already_exists_in_db() -> Result<(), LsError> {
    tokio_test(async {
        ...
        assert!(
            file_store
                .save_file(same_repository_name.to_owned(), same_file_path.clone(),  same_file_name.clone(), content_type.clone(), &binary_content.clone(),)
                .await
                .is_ok()
        );
        ...
        assert!(
            file_store
                .save_file(same_repository_name.to_owned(), format!("{same_file_name}-1"),  same_file_name.clone(), content_type.clone(), &binary_content.clone(),)
                .await
                .is_ok()
        );
        ...
        assert!(
            file_store
                .save_file("DB_TWO".to_owned(), same_file_path.clone(),  same_file_name.clone(), content_type.clone(), &binary_content.clone(),)
                .await
                .is_ok()
        );
...
fn should_fail_if_file_already_exists_in_fs() -> Result<(), LsError> {
    tokio_test(async {
        ...
        assert!(
            file_store
                .save_file("FS_TWO".to_owned(), same_file_path.clone(),  same_file_name.clone(), content_type.clone(), &binary_content.clone(),)
                .await
                .is_ok()
        );
...
fn should_read_all_file_data_by_repository() -> Result<(), LsError> {
    tokio_test(async {
        ...
        file_store
            .save_file(save_repository.to_owned(),file_name_1.clone(), file_name_1.clone(), content_type.clone(), &binary_content)
            .await?;
        ...
        file_store
            .save_file(save_repository.to_owned(),file_name_2.clone(),file_name_2.clone(), content_type.clone(), &binary_content)
            .await?;
        ...
        file_store.save_file( save_repository.to_owned(),file_name_3.clone(),file_name_3.clone(), content_type, &binary_content).await?;
...
fn should_return_if_file_exists_by_repository() -> Result<(), LsError> {
    tokio_test(async {
        ...
        let saved = file_store.save_file(save_repository.to_owned(), file_name.clone(), file_name, content_type, &binary_content).await?;
