...
#[tokio_shared::test]
async fn should_save_file_to_db() -> Result<(), LsFileStoreError> {
...
#[tokio_shared::test]
async fn should_save_file_to_fs() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_save_file_to_db_with_specific_repo() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_save_file_to_fs_with_specific_repo() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn save_should_fails_if_fs_repo_does_not_exist() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_save_file_to_db_with_relative_folder() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_save_file_to_fs_with_relative_folder() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_save_file_to_db_with_relative_folder_in_repository() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_save_file_to_fs_with_relative_folder_in_repository() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_delete_file_from_db() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_delete_file_from_fs() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_allow_files_with_same_path_but_different_repository() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_fail_if_file_already_exists_in_db() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_fail_if_file_already_exists_in_fs() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn should_read_all_file_data_by_repository() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
    ...
    file_store
        .save_file(save_repository.to_owned(), file_name_3.clone(), file_name_3.clone(), content_type, &binary_content)
        .await?;
...
#[tokio_shared::test]
async fn should_return_if_file_exists_by_repository() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let file_store = &data.0.file_store_service;
...
#[tokio_shared::test]
async fn save_file_should_reject_payload_above_save_max_size_bytes() -> Result<(), LsFileStoreError> {
    let data = data(false).await;
    let module = &data.0;
    ...
    for repo in &["DB_ONE", "FS_ONE"] {
        ...
        let saved = capped_service
            .save_file((*repo).to_owned(), path.clone(), path, "application/octet-stream".to_owned(), &small_content)
            .await?;
