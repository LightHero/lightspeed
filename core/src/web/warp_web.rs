...
pub fn req_info() -> impl Filter<Extract = (ReqInfo,), Error = Rejection> + Clone {
    warp_external::method()
        .and(path::full())
        .and(header::optional::<String>(JWT_TOKEN_HEADER))
        .map(
            |method: http::Method, path: path::FullPath, auth_header: Option<String>| ReqInfo {
                method,
                path,
                auth_header,
            },
    ...
    fn into_response(self) -> Response {
        match self {
            ...
            LightSpeedError::ValidationError { details } => {
                ...
                let error_details =
                    WebErrorDetails::from_error_details(http_code.as_u16(), &details);
            ...
            LightSpeedError::BadRequest { code, .. } => {
                ...
                let error_details = reply::json(&WebErrorDetails::from_message(
                    http_code.as_u16(),
                    &Some(code.to_string()),
                ));
