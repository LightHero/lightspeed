...
#[inline]
pub fn validate_ip_v4<S: Into<String>>(error_details: &ErrorDetails, field_name: S, val: &str) {
    if !validator::validate_ip_v4(val) {
        error_details.add_detail(field_name, ErrorDetail::new(NOT_VALID_IP, vec![]))
    }
}

/// Validates whether the given string is an IP V6
#[inline]
pub fn validate_ip_v6<S: Into<String>>(error_details: &ErrorDetails, field_name: S, val: &str) {
...
#[inline]
pub fn validate_ip<S: Into<String>>(error_details: &ErrorDetails, field_name: S, val: &str) {
...
mod tests {
    ...
    fn should_validate_generic_ip_v4_and_return_no_errors() {
        let error_details = ErrorDetails::new();
        validate_ip(&error_details, "ip", "127.0.0.1");
        assert!(error_details.details().borrow().is_empty())
    }
    ...
    fn should_validate_ip_and_return_errors() {
        let error_details = ErrorDetails::new();
        validate_ip(&error_details, "ip", "127.0.0.1.1");
        assert_eq!(1, error_details.details().borrow().len());
        assert_eq!(
            ErrorDetail::new(NOT_VALID_IP, vec![]),
            error_details.details().clone().borrow()["ip"][0]
        )
    }
    ...
    fn should_validate_ip4_and_return_no_errors() {
        let error_details = ErrorDetails::new();
        validate_ip_v4(&error_details, "ip", "127.0.0.1");
        assert!(error_details.details().borrow().is_empty())
    }
    ...
    fn should_validate_ip4_and_return_errors() {
        let error_details = ErrorDetails::new();
        validate_ip_v4(&error_details, "ip", "127.0.0.1.1");
        assert_eq!(1, error_details.details().borrow().len());
        assert_eq!(
            ErrorDetail::new(NOT_VALID_IP, vec![]),
            error_details.details().clone().borrow()["ip"][0]
