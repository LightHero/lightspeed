...
impl JwtService {
    ...
    pub fn generate_from_payload<'a, T: serde::ser::Serialize>(&self, payload: &'a T) -> Result<(JWT<&'a T>, String), LightSpeedError> {
    ...
    pub fn parse_token<T: serde::de::DeserializeOwned>(&self, jwt_string: &str) -> Result<JWT<T>, LightSpeedError> {
        ...
        match result {
            Ok(t) => Ok(t.claims),
            Err(e) => match *e.kind() {
                jsonwebtoken::errors::ErrorKind::ExpiredSignature => Err(LightSpeedError::ExpiredTokenError { message: e.to_string() }),
...
mod test {
    ...
    fn should_create_jwt_string_from_token() {
        ...
        let token = super::JWT { payload, sub: "".to_string(), exp: Local::now().timestamp() + 3600, iat: Local::now().timestamp() };
