...
#[tokio_shared::test]
async fn executor_does_not_fire_when_not_due() {
...
#[tokio_shared::test]
async fn executor_fires_and_advances_when_due() {
    let d = data(false).await;
    let count = Arc::new(AtomicUsize::new(0));
    let executor = JobExecutor::new_with_utc_tz(d.0.clone());
    executor
...
#[tokio_shared::test]
async fn executor_fires_multiple_jobs_in_one_tick() {
    let d = data(false).await;
...
#[tokio_shared::test]
async fn two_executors_with_same_group_and_name_fire_only_once() {
...
async fn failing_job_keeps_schedule_due() {
    ...
    executor.add_job_with_scheduler(due_now(), Job::new(group.clone(), name.clone(), None, FailingTask)).await.unwrap();
