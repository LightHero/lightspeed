...
async fn many_concurrent_claimers_on_one_due_row() {
...
#[tokio_shared::test]
#[cfg_attr(feature = "sqlite", ignore = "sqlite: single-connection pool serialises naturally")]
async fn many_workers_many_due_rows_each_claimed_once() {
    let d = data(false).await;
    let repo = d.0.clone();
    let group = unique_name();
    ...
    assert_eq!(claims.load(Ordering::SeqCst), M, "every due row must be claimed exactly once across {WORKERS} workers",);
}
...
#[tokio_shared::test]
#[cfg_attr(feature = "sqlite", ignore = "sqlite: single-connection pool serialises naturally")]
async fn concurrent_claims_on_different_rows_run_in_parallel() {
    let d = data(false).await;
    let repo = d.0.clone();
    let group = unique_name();
...
#[tokio_shared::test]
#[cfg_attr(feature = "sqlite", ignore = "sqlite: single-connection pool serialises naturally")]
async fn held_claim_does_not_starve_independent_row() {
    let d = data(false).await;
    let repo = d.0.clone();
    let group = unique_name();
    ...
    assert!(fast_claimed, "row `fast` should have been claimable while `slow` was locked (took {attempts} attempts)",);
}
