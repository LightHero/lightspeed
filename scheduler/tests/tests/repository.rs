...
#[tokio_shared::test]
async fn repository_can_be_initialised_twice() {
...
#[tokio_shared::test]
async fn register_inserts_row_then_no_ops() {
...
#[tokio_shared::test]
async fn try_claim_due_returns_row_when_due() {
    let d = data(false).await;
    let repo = &d.0;
    let group = unique_name();
    let name = unique_name();
...
async fn for_update_skip_locked_serialises_concurrent_claims() {
    let d = data(false).await;
    let repo = &d.0;
    let group = unique_name();
    let name = unique_name();
...
async fn advance_updates_next_and_last_run() {
    ...
    let row = repo.try_claim_due(&mut tx2, &group, &name).await.unwrap().expect("after backdating, row must be due");
    assert!(row.last_run_at.is_some());
    repo.commit(tx2).await.unwrap();
}

#[tokio_shared::test]
async fn rollback_releases_lock_without_advance() {
    let d = data(false).await;
    let repo = &d.0;
    let group = unique_name();
    let name = unique_name();
...
#[tokio_shared::test]
async fn time_until_next_due_uses_db_clock_and_respects_groups() {
    let d = data(false).await;
    let repo = &d.0;
    let group = unique_name();
