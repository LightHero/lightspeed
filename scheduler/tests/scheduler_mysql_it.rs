...
async fn init() -> SharedFixture {
    ...
    let repo = MySqlScheduleRepository::init(c3p0).await.expect("constructor must apply migrations");
    (repo, node)
}
...
pub async fn start_repo() -> RepoUnderTest {
    ...
    MySqlScheduleRepository::init(c3p0).await.expect("init must be idempotent against the same pool")
}
