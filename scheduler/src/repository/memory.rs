...
mod tests {
    ...
    async fn register_is_idempotent() {
        ...
        repo.register(GRP, "a", SystemTime::now() + Duration::from_secs(3600), "")
            .await
            .unwrap();
    ...
    async fn same_name_in_different_groups_are_isolated() {
        let repo = MemoryScheduleRepository::init();
        repo.register("app-1", "cleanup", due_in_past(), "")
            .await
            .unwrap();
        repo.register(
            "app-2",
        ...
        let mut tx = repo.begin().await.unwrap();
        assert!(
            repo.try_claim_due(&mut tx, "app-1", "cleanup")
                .await
                .unwrap()
                .is_some(),
        ...
        assert!(
            repo.try_claim_due(&mut tx, "app-2", "cleanup")
                .await
                .unwrap()
                .is_none(),
    ...
    async fn try_claim_due_returns_none_when_not_due() {
        let repo = MemoryScheduleRepository::init();
        repo.register(GRP, "a", SystemTime::now() + Duration::from_secs(60), "")
            .await
            .unwrap();
    ...
    async fn time_until_next_due_returns_smallest_positive_or_zero() {
        let repo = MemoryScheduleRepository::init();
        let now = SystemTime::now();
        repo.register(GRP, "a", now + Duration::from_secs(60), "")
            .await
            .unwrap();
        repo.register(GRP, "b", now + Duration::from_secs(10), "")
            .await
            .unwrap();
        ...
        let d = repo
            .time_until_next_due(&[(GRP, "a"), (GRP, "b")])
            .await
            .unwrap()
            .expect("at least one matching row");
        ...
        let d = repo
            .time_until_next_due(&[(GRP, "past")])
            .await
            .unwrap()
            .expect("`past` must be found");
        assert_eq!(d, Duration::ZERO);

        // Unknown pair returns None.
        assert!(
            repo.time_until_next_due(&[(GRP, "missing")])
                .await
                .unwrap()
                .is_none(),
        );
        ...
        assert!(
            repo.time_until_next_due(&[("wrong-group", "a")])
                .await
                .unwrap()
                .is_none(),
        );
    ...
    async fn advance_updates_next_and_last_run() {
        ...
        repo.advance(&mut tx, GRP, "a", future, SystemTime::now())
            .await
            .unwrap();
        repo.commit(tx).await.unwrap();
        ...
        let mut tx2 = repo.begin().await.unwrap();
        assert!(
            repo.try_claim_due(&mut tx2, GRP, "a")
                .await
                .unwrap()
                .is_none(),
        );
        // Advance back into the past — now claimable again.
        repo.advance(&mut tx2, GRP, "a", due_in_past(), SystemTime::now())
            .await
            .unwrap();
        let row = repo
            .try_claim_due(&mut tx2, GRP, "a")
            .await
            .unwrap()
    ...
    async fn register_with_different_fingerprint_reanchors_next_run_at() {
        ...
        repo.register(GRP, "a", original_next, "interval:3600.000000000:false")
            .await
            .unwrap();

        let new_next = SystemTime::now() + Duration::from_secs(60);
        repo.register(GRP, "a", new_next, "interval:60.000000000:false")
            .await
            .unwrap();
        ...
        repo.advance(&mut tx, GRP, "a", due_in_past(), SystemTime::now())
            .await
            .unwrap();
