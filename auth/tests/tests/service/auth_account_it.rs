        ...
        auth_module
            ...
            .transaction(|mut conn| async move {
                let user_by_id = auth_module
                    .auth_account_service
                    .fetch_by_user_id_with_conn(&mut conn, user.id)
                    .await?;
...
fn should_add_and_remove_roles() -> Result<(), LightSpeedError> {
    test(async {
        ...
        let user = auh_service
            .delete_roles(user.id, &vec!["one".to_owned()])
            .await?;
        assert!(user.data.roles.is_empty());

        let user = auh_service
            .add_roles(user.id, &vec!["one".to_owned()])
            .await?;
        assert_eq!(vec!["one".to_owned()], user.data.roles);

        let user = auh_service
            .add_roles(user.id, &vec!["two".to_owned(), "three".to_owned()])
            .await?;
        assert_eq!(
        ...
        let user = auh_service
            .delete_roles(user.id, &vec!["two".to_owned(), "four".to_owned()])
            .await?;
        assert_eq!(vec!["one".to_owned(), "three".to_owned()], user.data.roles);

        let user = auh_service
            .delete_roles(user.id, &vec!["one".to_owned(), "three".to_owned()])
            .await?;
        assert!(user.data.roles.is_empty());
