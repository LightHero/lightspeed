...
#[test]
fn should_assign_default_roles_at_account_creation() {
    test(false, |auth_module| {
        let username = new_hyphenated_uuid();
        let email = format!("{}@email.fake", username);
        let password = new_hyphenated_uuid();
...
#[test]
fn should_use_the_email_as_username_if_not_provided() {
    test(false, |auth_module| {
...
#[test]
fn should_use_the_email_as_username_if_username_is_empty() {
    test(false, |auth_module| {
        let email = format!("{}@email.fake", new_hyphenated_uuid());
        let password = new_hyphenated_uuid();
...
#[test]
fn should_regenerate_activation_token() {
    test(false, |auth_module| {
        let (user, token) = create_user(&auth_module, false)?;
...
#[test]
fn should_generate_reset_password_token() {
    test(false, |auth_module| {
        let (user, _) = create_user(&auth_module, true)?;
...
#[test]
fn should_add_and_remove_roles() {
    test(false, |auth_module| {
