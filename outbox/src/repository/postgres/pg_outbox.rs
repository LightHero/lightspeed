...
impl OutboxRepository for PgOutboxRepository {
    ...
    async fn fetch_all_by_type_and_status_for_update<D: Send + Sync + Unpin + Serialize + DeserializeOwned>(&self, tx: &mut PgConnection, r#type: &str, status: OutboxMessageStatus, limit: usize) -> Result<Vec<OutboxMessageModel<D>>, OutboxError> {
    ...
    async fn update<D: Send + Sync + Unpin + Serialize + DeserializeOwned>(&self, tx: &mut PgConnection, data: OutboxMessageModel<D>) -> Result<OutboxMessageModel<D>, OutboxError> {
        Ok(tx.update(data).await?)
    }

    /// Saves a new outbox message
    async fn save<D: Send + Sync + Unpin + Serialize + DeserializeOwned>(&self, tx: &mut PgConnection, data: OutboxMessageData<D>) -> Result<OutboxMessageModel<D>, OutboxError> {
