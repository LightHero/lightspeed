...
impl<RepoManager: OutboxRepositoryManager> LsOutboxService<RepoManager> {
    pub fn new(auth_config: OutboxConfig, outbox_repo: &RepoManager) -> Self {
        LsOutboxService {
            outbox_config: auth_config,
            repo: outbox_repo.outbox_repo(),
            c3p0: outbox_repo.c3p0().clone(),
        }
    }

    /// Saves a new outbox message
    pub async fn save<D: Send + Sync + Unpin + Serialize + DeserializeOwned>(
        &self,
        tx: &mut <RepoManager::DB as Database>::Connection,
    ...
    pub fn channel<
        D: Send + Sync + Unpin + Serialize + DeserializeOwned,
        F: 'static
            + Send
            + Sync
    ...
    >(
        ...
        r#type: &str,
        callback: F,
    ) -> (OutboxSender<RepoManager, D>, OutboxReceiver<RepoManager, D>) {
...
impl<RepoManager: OutboxRepositoryManager, D: Send + Sync + Unpin + Serialize + DeserializeOwned>
    OutboxSender<RepoManager, D>
{
    /// Creates a new OutboxSender
    pub fn new(repo: RepoManager::OutboxRepo, r#type: String) -> Self {
        Self { repo, r#type, phantom_d: std::marker::PhantomData }
    }
        ...
        let mut messages = {
            self.c3p0
                .transaction::<_, OutboxError, _>(async |tx| {
                    let messages = self
                        .repo
                        .fetch_all_by_type_and_status_for_update::<D>(
                            tx,
                            &self.r#type,
