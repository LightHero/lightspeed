...
use lightspeed_outbox::{
    error::OutboxError, model::{OutboxMessage, OutboxMessageData, OutboxMessageStatus}, repository::{OutboxRepository, OutboxRepositoryManager}
};
...
fn test_outbox_callback_success() -> Result<(), LsError> {
    tokio_test(async {
        ...
        // Arrange
            ...
            // Act
        let message = c3p0.transaction::<_, OutboxError, _>(async |tx| {
...
async fn assert_message_status(repo_manager: &RepoManager, id: u64, status: OutboxMessageStatus) {
    repo_manager.c3p0().transaction::<_, OutboxError, _>(async |tx| {
