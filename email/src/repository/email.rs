...
pub fn new(email_config: EmailClientConfig) -> Result<Arc<dyn EmailClient>, LsEmailError> {
    ...
    if let Some(recipients) = email_config.forward_all_emails_to_fixed_recipients {
        ...
        if recipients.is_empty() {
            Err(LsEmailError::ConfigurationError {
...
impl FromStr for EmailClientType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(LsEmailError::ConfigurationError { message: format!("Unknown Email client_type [{s}]") }),
        }
    }
}
