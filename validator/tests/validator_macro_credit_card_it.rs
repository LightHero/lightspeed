...
fn credit_card_accepts_well_known_test_numbers() {
    for ok in ["4532015112830366", "5425233430109903", "374245455400126"] {
        let v = PaymentValidable::new(Payment { card_number: ok.to_string(), untouched: String::new() });
...
fn credit_card_accepts_grouped_numbers_with_spaces_and_dashes() {
    let v = PaymentValidable::new(Payment { card_number: "4532 0151 1283 0366".to_string(), untouched: String::new() });
...
fn credit_card_rejects_luhn_invalid_number() {
    let v = PaymentValidable::new(Payment { card_number: "1234567890123456".to_string(), untouched: String::new() });
...
fn credit_card_rejects_non_digit_characters() {
    let v = PaymentValidable::new(Payment { card_number: "4532abcd1283ZZZZ".to_string(), untouched: String::new() });
    let returned = match v.validate() {
        Ok(_) => panic!("expected Err"),
        Err(v) => v,
    };
...
fn credit_card_rejects_too_short_or_too_long() {
    ...
    for bad in ["000000000000", "45320151128303661234"] {
        let v = PaymentValidable::new(Payment { card_number: bad.to_string(), untouched: String::new() });
        let returned = match v.validate() {
...
fn credit_card_validator_works_on_cow_str_field() {
    let v = CowStringFieldsValidable::new(CowStringFields { card_number: Cow::Borrowed("4532015112830366") });
    assert!(v.validate().is_ok());

    let v = CowStringFieldsValidable::new(CowStringFields { card_number: Cow::Owned("nope".to_string()) });
    let returned = match v.validate() {
        Ok(_) => panic!("expected Err"),
        Err(v) => v,
    };
...
fn macro_attaches_one_validator_per_credit_card_attribute() {
    let v = PaymentValidable::new(Payment { card_number: "4532015112830366".to_string(), untouched: String::new() });
