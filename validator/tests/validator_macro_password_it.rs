...
fn default_policy_accepts_strong_password() {
    let v = DefaultPolicyValidable::new(DefaultPolicy { password: "Aa1!xyz".to_string(), untouched: String::new() });
    assert!(v.validate().is_ok());
}

#[test]
fn default_policy_collects_every_missing_class_in_one_error() {
    let v = DefaultPolicyValidable::new(DefaultPolicy { password: "aaaa".to_string(), untouched: String::new() });
...
fn default_policy_rejects_trailing_whitespace() {
    ...
    assert_eq!(returned.password.errors(), &[pw_err(&[PasswordViolation::HasTrailingWhitespace])],);
}
...
fn custom_special_chars_string_restricts_accepted_set() {
    ...
    let v = CustomSpecialCharsValidable::new(CustomSpecialChars { password: "abc*def".to_string() });
    assert!(v.validate().is_ok());
    ...
    assert_eq!(returned.password.errors(), &[pw_err(&[PasswordViolation::MissingSpecialChar])],);
}

#[test]
fn trailing_whitespaces_true_allows_trailing_space() {
    let v = AllowsTrailingWhitespaceValidable::new(AllowsTrailingWhitespace { password: "Aa1!xyz ".to_string() });
    assert!(v.validate().is_ok());
}

#[test]
fn password_validator_works_on_cow_str_field() {
    let v = CowStringFieldsValidable::new(CowStringFields { password: Cow::Borrowed("Aa1!xyz") });
    assert!(v.validate().is_ok());

    let v = CowStringFieldsValidable::new(CowStringFields { password: Cow::Owned("aaaa".to_string()) });
    let returned = match v.validate() {
        Ok(_) => panic!("expected Err"),
        Err(v) => v,
    };
