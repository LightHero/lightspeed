...
#[tokio_shared::test]
async fn should_create_pending_user() -> Result<(), LsAccountManagementError> {
...
#[tokio_shared::test]
async fn should_assign_default_roles_at_account_creation() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_return_user_by_id() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_return_user_by_username() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_use_the_email_as_username_if_not_provided() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_use_the_email_as_username_if_username_is_empty() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_activate_user() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_activate_user_only_if_activation_token_type() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_activate_user_only_if_pending_activation() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_regenerate_activation_token_by_email_and_username() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
    let (user, token) = create_user(auth_module, false).await?;
...
///   * `LsAccountManagerError::NotFound` — username unknown,
...
#[tokio_shared::test]
async fn generate_new_activation_token_should_use_uniform_error_for_all_failure_modes()
...
#[tokio_shared::test]
async fn should_regenerate_activation_token_by_email_and_username_even_if_token_expired()
-> Result<(), LsAccountManagementError> {
...
#[tokio_shared::test]
async fn should_login_active_user() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_not_login_inactive_user() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_return_wrong_credentials_on_login_of_inactive_user_with_wrong_password()
-> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_not_login_with_wrong_username() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
    let (user, _) = create_user_with_password(auth_module, password, true).await?;
...
#[tokio_shared::test]
async fn should_not_login_with_wrong_password() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
    let (user, _) = create_user_with_password(auth_module, password, true).await?;
...
#[tokio_shared::test]
async fn create_user_should_fail_if_username_not_unique() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn create_user_should_fail_if_email_not_unique() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_reset_password_by_token() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_fail_resetting_password_if_token_expired() -> Result<(), LsAccountManagementError> {
...
#[tokio_shared::test]
async fn should_reset_password_only_if_correct_token_type() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_reset_password_only_if_user_is_active() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_generate_reset_password_token() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;

    let (user, _) = create_user(auth_module, true).await?;

    let (new_user, token) = auth_module.auth_account_service.generate_reset_password_token(&user.data.username).await?;
    assert_eq!(user.id, new_user.id);
...
#[tokio_shared::test]
async fn should_not_generate_reset_password_token_if_user_not_active() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_change_user_password() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_not_change_user_password_if_wrong_old_password() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_not_change_user_password_if_inactive_user() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_add_and_remove_roles() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_change_username() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_change_email() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_change_username_and_email() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_disable_an_active_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_fail_disabling_a_pending_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_fail_disabling_a_disabled_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_activate_a_disabled_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_fail_reactivating_a_pending_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_fail_reactivating_an_active_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_delete_a_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_not_fail_deleting_a_deleted_user() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
    let password = "123456789";
...
#[tokio_shared::test]
async fn should_return_users_by_status() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_return_users_by_status_with_offset_and_limit() -> Result<(), LsAccountManagementError> {
    // Arrange
    let data = data(true).await;
    let auth_module = &data.0;
    ...
    // Act
    let all_users =
        auth_module.auth_account_service.fetch_all_by_status(AccountStatus::Active, user_1.id, u32::MAX).await.unwrap();

    let offset_one_users =
        auth_module.auth_account_service.fetch_all_by_status(AccountStatus::Active, user_2.id, u32::MAX).await.unwrap();
...
#[tokio_shared::test]
async fn should_fail_login_when_password_expired() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn should_login_when_password_within_expiration_window() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn change_password_should_reset_password_age() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
...
#[tokio_shared::test]
async fn reset_password_should_reset_password_age() -> Result<(), LsAccountManagementError> {
    let data = data(false).await;
    let auth_module = &data.0;
