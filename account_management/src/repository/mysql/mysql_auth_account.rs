...
impl AuthAccountRepository for MySqlAuthAccountRepository {
    ...
    async fn fetch_by_id(&self, tx: &mut MySqlConnection, user_id: i64) -> Result<AuthAccountModel, LsAccountManagerError> {
        Ok(tx.fetch_one_by_id::<AuthAccountData>(user_id).await?)
    }

    async fn fetch_by_username(&self, tx: &mut MySqlConnection, username: &str) -> Result<AuthAccountModel, LsAccountManagerError> {
    ...
    async fn update(&self, tx: &mut MySqlConnection, model: AuthAccountModel) -> Result<AuthAccountModel, LsAccountManagerError> {
        Ok(tx.update(model).await?)
    }

    async fn delete(&self, tx: &mut MySqlConnection, model: AuthAccountModel) -> Result<AuthAccountModel, LsAccountManagerError> {
