...
impl SchemaRepository for PostgresSchemaRepository {
    ...
    async fn exists_by_name_and_project_id(
        ...
        let res =
            ::sqlx::query(sql).bind(name).bind(project_id as i64).fetch_one(tx).await.and_then(|row| row.try_get(0))?;

        Ok(res)
    }

    async fn save(&self, tx: &mut PgConnection, model: NewRecord<SchemaData>) -> Result<Record<SchemaData>, LsError> {
        Ok(tx.save(model).await?)
    }

    async fn update(&self, tx: &mut PgConnection, model: Record<SchemaData>) -> Result<Record<SchemaData>, LsError> {
        Ok(tx.update(model).await?)
    }

    async fn delete(&self, tx: &mut PgConnection, model: Record<SchemaData>) -> Result<Record<SchemaData>, LsError> {
    ...
    async fn delete_by_project_id(&self, tx: &mut PgConnection, project_id: u64) -> Result<u64, LsError> {
        ...
        Ok(::sqlx::query(sql).bind(project_id as i64).execute(tx).await?.rows_affected())
    }
}
