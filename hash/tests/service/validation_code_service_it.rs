...
async fn should_generate_validation_code() -> Result<(), LsError> {
    ...
    let verify_code_response = validation_code_service.verify_validation_code(verify_code_request.clone(), true)?;
    ...
    assert!(!validation_code_service.verify_validation_code(bad_verify_code, true).unwrap().code_valid);
